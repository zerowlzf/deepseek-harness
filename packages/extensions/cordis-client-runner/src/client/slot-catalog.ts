...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.chat.turn-stats\', () => ctx.slots.register(\n      { name: \'conversation.chat.turn-stats\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.composer.dock\' (client-ui-chat), so it exists while that entry is mounted',
    occupants: [
