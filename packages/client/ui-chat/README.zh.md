...
该页脚由三个槽位扩展，它们都不是第二条操作行。`conversation.chat.turnTail` 在操作行之上渲染功能贡献；`conversation.chat.turnEndInfo` 在操作行的行末信息簇内渲染读数——位于随包提供的用量触发之后、时钟之前——因此轮次自身账目已说明的数字与它所属的用量数字并排，并跟随该操作行的显隐；`conversation.chat.assistant-actions` 在复制与分支图标之间渲染控件。在产生任何定稿文本之前被中断的轮次只渲染 tail：没有复制、没有分支、没有用量触发、没有时钟，也没有行末信息簇。
