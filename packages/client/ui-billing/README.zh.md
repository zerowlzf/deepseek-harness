...
三个界面，卸载时各自还原：`settings.section`（计费页）、`conversation.composer.stats`（官方统计行内部的两个胶囊），以及 `conversation.chat.turnTail` 链（「已完成轮次」操作条里的每轮费用胶囊，链优先级低于官方交付文件条目）。计费页为每个「用户确实配置过」的提供方（以及仍被某条已存单价行点名的提供方）给出一张卡片，模型列表收在该卡片自己的编辑控件之后，因此没人配置过的目录条目不会带来任何单价行。
