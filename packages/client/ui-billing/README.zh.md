...
composer 行把本会话费用与余额放进官方轮次/步数胶囊与 token 胶囊自己那一行：`conversation.composer.stats` 是 ui-chat 统计行亲自渲染的一个洞，分隔线只在洞里有内容时才画，因此这些数字与官方那组共享同一份居中，而不是落在它旁边。那一行是**宽度受限**的——680px 封顶时内容盒只有 616px，其中官方两个胶囊、分隔线与行内间距已占 409px——因此两个数字都只显示金额本身（`¥12.75`、`余额 ¥16.78`），间距 8px，不带图标装饰；它们的含义由无障碍名与 hover 提示承担。会话合计累积 `tokenUsage` 投影——整份持久日志，而不是已加载窗口——按运行总量每次增长时生效的路由计价，这正是让会话中途换模型能被正确切分的原因。每个轮次在「已完成轮次」操作条里得到自己的费用胶囊：以该轮次的持久账目为总额，用已加载的尝试按路由拆分；若该轮次的助手行已不在窗口内，则回落到会话投影。被中断的轮次同样保留这条操作条：没有收尾消息就没有可复制、可分支的目标，但账目与费用胶囊正是这一行存在的理由。
