...
The composer's ambient dock carries a session-cost figure and a balance figure: `conversation.composer.dock` is the list ui-conversation declares for ambient entries under the composer card, and the shipped stats row is seated there as one entry, so these figures join it as another rather than reaching inside it. Both are bare amounts (`¥16.82` and `¥15.96`, distinguished by the coin and wallet glyphs, and a bare `-` while one is unknown); each one states what it is through its accessible name and hover title rather than through visible words. The session total accumulates the `tokenUsage` projection — the whole durable log, not the loaded window — by pricing each growth of the running total at the route active when it grew, which is what keeps a mid-session model switch correctly split.

Each completed Turn carries its own cost pill in its tail, through `conversation.chat.turnTail` — the list ui-chat declares for feature contributions before that Turn's action row, whose owner share is the Turn, its closing sequence, and the file opener. It shows `费用 ¥0.42` and opens that Turn's breakdown: one row per route, priced from the Turn's durable accounting and attributed by the route each loaded attempt was billed on. A Turn that also produced files shows both tail entries — the shipped files row and this figure — because the tail is a list, so neither displaces the other. Three cases carry no figure, and the dialog says which one applies:
...
An interrupted Turn keeps its tail too: no closing message means no copy or branch target, but the accounting and the cost pill are what that tail is for. A Turn that has not ended yet has no tail — the shipped tail node appears when the Turn closes — so the newest answer carries its cost only once that Turn settles.
...
### The live configuration fields

The values are this plugin's own configuration, declared live in its `Config` schema and held in the Profile's plugin configuration under the entry id `ui-billing` — the row id the web composition declares. That is also why a document written before the move to Profile configuration migrates into them instead of being lost: the legacy `settings.yaml` section carries the same name.
...
- The cost figure is a tail contribution (`conversation.chat.turnTail`), the list ui-chat declares for feature contributions before a completed Turn's action row, and a list is what keeps it: the shipped produced-files entry and this figure coexist for the same Turn. The per-attempt node kind is `assistant-step`, and its `finalNode.provenance` and `finalNode.time` are the route and the moment that attempt was billed on.
...
- The configuration entry (`ui-billing`) and the copy dictionary (`billing`) stay separately named: one identifier for both binds the scope to the dictionary, so every surface renders its unavailable state while the Host serves correct values.
- The package is inside the per-file 100% coverage gate, and one arm carries a `/* v8 ignore */`: the turn fold's last-row guard, whose row every attempt above added, with a turn that has no attempt returning before it. A release tag waits for that gate beside `pnpm run doc-sync`; between releases a change is held to the package suite, both TypeScript faces, and the linter, because review attention buys more there than the last few percent.
- A read the page asks for travels as a write to the live fields rather than a call into the Host: the browser cannot read the documentation page itself (its origin sends no grant for one) and this package owns no Remote namespace to call, so `officialRequest` is the channel both halves share. The client writes the moment it asked; the Host reads the page and clears the field with that read's own settlement. A volatile-only change commits into the running instance without a remount, so the Host learns of the request through `loader/volatile-update` rather than by being restarted. The field being absent and being null are the same thing to every reader, because a union in the schema resolves neither into the stored document.
